...
class Board extends Model
{
    ...
    public function setByData($data)
    {
        if (request()->route()->getName() != 'dbTransfer' /* 데이터 이전 작업아닐때 */) {
            ...
            if (!empty($data['field'])) {
                // 'ALL' 값(전체 체크)을 빼고 나머지만 저장
                $fields = array_diff($data['field'], ['ALL']);
            ...
            $this->code = $data['code'];
            $this->user_sid = $data['user_sid'] ?? 0;
            $this->realfile1 = $data['realfile1'] ?? null;
            $this->filename1 = $data['filename1'] ?? null;
            $field = $data['field'] ?? null;
            $this->thumbnail_realfile = $data['thumbnail_realfile'] ?? null;
            $this->thumbnail_filename = $data['thumbnail_filename'] ?? null;
        }
...
//        $this->notice_eDate = $data['notice_eDate'] ?? null;
        ...
        $this->event_sTime = $data['event_sTime'] ?? null;
        $this->event_eDate = ($this->date_type == 'D') ? null : $data['event_eDate'];
        $this->event_eTime = $data['event_eTime'] ?? null;
    ...
    public function isNew($hour = 72) // 기본 72 시간 or 변수시간 기준으로 신규게시글 체크
    {
    ...
    {
        $sDate = date('Y.m.d', strtotime($this->event_sDate));
    ...
    public function isHeart($u_sid = null)
    {
        return Heart::where(['bsid'=>$this->sid, 'user_sid'=>$u_sid])->exists();
    }

    // Board 모델에서
    public function hearts()
    {
        return $this->hasMany(Heart::class, 'bsid');
    }
}
