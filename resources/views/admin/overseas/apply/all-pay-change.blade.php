...
@section('contents')
    <div style="padding: 25px;">
        <div class="write-form-wrap">
            <form id="all-judge-change-frm" method="post" data-case="all-pay-change">
                <fieldset>
                    <h2>지급여부 상태 일괄 변경 팝업</h2>
                    <ul class="list-type list-type-bar mt-20">
                        <li>지급여부 선택 후 참가자를 선택하여 [완료] 버튼을 클릭 해주셔야 완료 됩니다.</li>
                    </ul>

                    <ul class="write-wrap mt-20">
                        <li>
                            <div class="form-tit"><strong class="required">*</strong> 지급여부</div>
                            <div class="form-con">
                                <div class="radio-wrap cst">
                                    @foreach($overseasConfig['pay_result'] as $tkey=>$tval)
                                        <label class="radio-group">
                                            <input type="radio" name="pay_result" id="pay_result_{{ $tkey }}" value="{{ $tkey }}">{{ $tval }}
                                        </label>
                                    @endforeach
                                </div>
                            </div>
                        ...
                        <table class="cst-table application-table text-center">
                            ...
                            @forelse($list as $row)
                                <tr>
                                    ...
                                    <td>{{ $row->email }}</td>
                                    <td>{{ $row->phone }}</td>
                                    <td>{{ $overseasConfig['pay_result'][$row->pay_result] ?? '' }}</td>
                                </tr>
                            @empty
                                <tr>
...
@section('addScript')
    <script>
        ...
        $(document).on('submit', form, function () {
            if (isEmpty($('input[name="pay_result"]:checked').val())) {
                alert('지급여부를 선택해주세요.');
                $('input[name="pay_result"]').eq(0).focus();
                return false;
            }
