...
{{--                                                    <input type="time" name="event_eTime" id="event_eTime" class="form-item eDate-display" value="{{ $board->event_eTime ?? '' }}">--}}
                                    ...
                                    <li>
                                        <div class="form-tit">초록집</div>
                                        <div class="form-con">
                                            <div class="radio-wrap cst">
                                                <label class="radio-group" for="choice_type_A"><input type="radio" name="choice_type" id="choice_type_A" value="A" {{ ($board->choice_type ?? '') == 'A' ? 'checked' : '' }}>파일첨부</label>
                                                <label class="radio-group" for="choice_type_B"><input type="radio" name="choice_type" id="choice_type_B" value="B" {{ ($board->choice_type ?? '') == 'B' ? 'checked' : '' }}>링크연결</label>
                                            </div>
                                            <div class="form-group mt-10">
                                                <input type="text" name="etc" id="etc" class="form-item" placeholder="홈페이지 주소를 입력해주세요." value="{{ $board->etc ?? '' }}" {{ ($board->choice_type ?? '') == 'A' ? 'disabled' : '' }}>
                                            </div>
                                            <div class="filebox mt-10">
                                                <input class="upload-name form-item" id="thumbnail_name" name="thumbnail_name" value="{{ $board->thumbnail_filename ?? '' }}" placeholder="파일첨부" readonly="readonly" {{ ($board->choice_type ?? '') == 'B' ? 'disabled' : '' }}>
                ...
                etc: {
                    isEmpty: {
                        depends: function(element) {
                            return $("input[name='choice_type']:checked").val()==='B';
                        },
                    },
                },
                thumbnail_name: {
                    isEmpty: {
                        depends: function(element) {
                            return $("input[name='choice_type']:checked").val()==='A';
                        },
                    },
                },
