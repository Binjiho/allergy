...
class AgentMailServices
{
    ...
    private $access_token = 'zUlE7si1IpqmcSgEHVqWKYrtkbjyLiyz4zG'; // 학회별 access token
                        ...
                        $query->where(function ($q) use ($searchKey) {
                            $q->orWhere('name_kr', 'like', "%{$searchKey}%")
                                ...
                                ->orWhere(function ($qq) use ($searchKey) {
                                    $qq->where('phone', 'like', "%{$searchKey}%")
                                        ->orWhereRaw("REPLACE(phone, '-', '') LIKE ?", ["%" . str_replace('-', '', $searchKey) . "%"]);
                                });
                        });
                    }
                    break;
...
//                dd($filterJson['column'], $key, $searchFilter );

//                if (empty($searchKeyword)) {
//                    continue;
//                }

                if (array_key_exists($key, $searchFilter) === false) {
                    continue;
                }

                $searchKeyword = $searchFilter[$key] ?? '';

                // 테이블 특정필드 예외 처리 쿼리 (예시)
//                if ($table === 'user_binfo' && $key === 'post') {
//                    $query->where($key, 'like', "%{$searchKeyword}%");
//                    continue;
//                }
//                if ($table === 'user_binfo' && $key === 'major_level') {
//                    $query->where('memberpart', '2');
//                    $query->whereIn('job_level1', $searchKeyword);
//                    continue;
//                }
...
//                        $quoteStringResult = $this->quoteString($searchKeyword);
                        $query->whereIn($key, $searchKeyword);
                        break;
    ...
    private function settingJson()
    {
        ...
        $json['template'] = [
            'T1' => [
                'name' => '템플릿 1',
                'path' => "{$this->domain}/api/mail/template/temp01", //파일명
            ],
			'T2' => [
                'name' => '템플릿 2',
