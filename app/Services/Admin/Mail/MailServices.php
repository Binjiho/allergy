...
class MailServices extends AppServices
{
    ...
    private function mailTypeSendService($mail)
    {
        ...
        // 회원등급별 발송
        if ($mail->send_type == '1') {
            $user = User::whereIn('level', $mail->level)->where('emailReception', 'Y')->get();
    ...
    private function mailSendRenewService(Request $request)
    {
        ...
        // 1. 필요한 wiseu_seq 목록 추출
        $wiseuSeqs = $mail->totMail()->where('status', 'R')->pluck('wiseu_seq')->toArray();
        ...
        try {
            ...
            // 3. 10개씩 끊어서 쿼리 실행
            foreach (array_chunk($wiseuSeqs, 200) as $chunk) {
                $placeholders = implode(',', array_fill(0, count($chunk), '?'));
                $sql = "SELECT * FROM NVECARESENDLOG 
                    WHERE ECARE_NO = ? AND CUSTOMER_KEY IN ($placeholders)";
                $stmt = $wiseUconnection->prepare($sql);

                $params = array_merge([$mailConfig['eCareNo']], $chunk);
                $stmt->execute($params);
            ...
            foreach ($allLogs as $row) {
                ...
                if (!empty($mailConfig['code'][$code])) {
                    MailSend::where('wiseu_seq', $customerKey)
                        ->update([
                            'status' => ($code === '250' || $code === '000') ? 'S' : 'F',
                            'status_msg' => $mailConfig['code'][$code],
                        ]);
                }
            }
