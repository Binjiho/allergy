...
<div class="popup-rolling-wrap js-popup-rolling inner-layer" style="position: fixed;top: 50%;left: 50%;transform: translate(-50%, -50%); z-index: 9999;">
    <div class="popup-contents type3" @if(empty($preview)) id="board-popup-{{ $board->sid }}" @endif style="width: auto; min-width:{{ $popup->width }}px; max-width:{{ $popup->width }}px; min-height:{{ $popup->height }}px; max-height:{{ $popup->height }}px; margin-top:{{ $popup->position_y }}px; margin-left:{{ $popup->position_x }}px; position: fixed;">
@endif
