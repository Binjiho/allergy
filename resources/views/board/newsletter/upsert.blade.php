                        ...
                        <form id="board-frm" onsubmit="return false;" data-sid="{{ $board->sid ?? 0 }}" data-case="board-{{ empty($board->sid) ? 'create' : 'update' }}">
                            <fieldset>
                                ...
                                <ul class="write-wrap">
                                    ...
                                    <li>
                                        <div class="form-tit"><strong class="required">*</strong> 연도 / 월</div>
                                        <div class="form-con">
                                            <div class="form-group n2 w-50p">
                                                <select name="year" id="year" class="form-item">
                                                    @foreach($boardConfig['year'] as $key => $val)
                                                        <option value="{{ $key }}" {{ $key == ($board->year ?? date('Y')) ? 'selected' : '' }}>{{ $val }}</option>
                                                    @endforeach
                                                </select>
                                                <select name="month" id="month" class="form-item">
                                                    @foreach($boardConfig['month'] as $key => $val)
                                                        <option value="{{ $key }}" {{ $key == ($board->month ?? date('m')) ? 'selected' : '' }}>{{ $val }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                    ...
                                    @if($boardConfig['use']['link'])
                                        <li>
                                            <div class="form-tit"><strong class="required">*</strong> 뉴스레터 URL</div>
